...
export const getWeatherBackground = (condition, isDay = null) => {
  const timeOfDay = isDay !== null ? (isDay ? 'Day' : 'Night') : (getTimeOfDay() === 'day' ? 'Day' : 'Night');
  ...
  if (normalizedCondition.includes('cloudy') || normalizedCondition.includes('overcast') || normalizedCondition.includes('partly')) {
  ...
  if (normalizedCondition.includes('snow') || normalizedCondition.includes('snowy')) {
